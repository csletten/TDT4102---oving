...
int xCoord(Point origo, int i, int maxMonthNo) {
...
int main() {
    vector<Temps> temp = createVector();
    vector<int> maxTemp;
    vector<int> minTemp;
    for (Temps t: temp){
        maxTemp.push_back(t.max);
        minTemp.push_back(t.min);
    }
    
    int maxMonthNo = maxTemp.size();
...
//    vector<int> minTemp{ 10, 9, 5, -2, -3, -4, -6, -6, 1, 8, 8, 13 };
    ...
    for (int i = 0; i < maxTemp.size(); i++) {
        oplMax.add(Point{ xCoord(origo,i, maxMonthNo), yCoord(origo, maxTemp[i], totalMin, ySpan) });
    }
    ...
    for (int i = 0; i < minTemp.size(); i++) {
        oplMin.add(Point{ xCoord(origo,i, maxMonthNo), yCoord(origo, minTemp[i], totalMin, ySpan) });
    }
